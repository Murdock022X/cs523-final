\begin{figure*}[t]
    ...
    \end{subfigure} 
    \caption{Rx stress test (multiple flows per core): \name{} consistently matches vIOMMU-off throughput.
    }
    \label{fig:stress}
\end{figure*}
...
{\name} follows the threat model of strict protection mode of Linux IOMMU~\cite{nadav2010iommu} and prior work~\cite{markuze2018damn,Amit2011vIOMMU,RajapakshaIOMMU}.
...
{\name} defends against attacks that control malicious or compromised IO devices.
...
% Intuitively, to achieve the same throughput, more pages will be activated by {\name}. 
We show that the memory overhead of \brand
for our evaluation setup is pretty small.
We run 24 Iperf flows for 120 seconds, sampling the number of active pages every 0.5 seconds for {\name} and vIOMMU-off.
\name's throughput is within 1\% of vIOMMU-off in these experiments.
...
\begin{figure}[!htbp]
    ...
    \begin{subfigure}[b]{0.49\columnwidth}
        ...
        \label{fig:tx_exp_memory_total}
    \end{subfigure}
    \caption{For Tx-server setup, {\name} incurs less than 3.3\%
...
\begin{figure}[!htbp]
    ...
    \begin{subfigure}[b]{0.49\columnwidth}
        ...
        \Description{Plot showing TX page usage over time.}
        \caption{Active pages required by the Tx datapath over time.}
        \label{fig:rx_exp_memory_tx}
    \end{subfigure}
    \begin{subfigure}[b]{0.49\columnwidth}
        \includegraphics[width=\columnwidth]{figures/rx_mem/mem_fig_tx_rx_active_pages.pdf}
        \Description{Plot showing RX+TX page usage over time.}
        \caption{Active pages required by the Rx and Tx datapath over time.}
        \label{fig:rx_exp_memory_total}
    \end{subfigure}
    \caption{For Rx-server setup, {\name} incurs less than 2.3\% additional memory overhead over vIOMMU-off.}
    \label{fig:rx_memory}
\end{figure}

\para{Tx-server.} We measure active physical pages for Tx held by the mlx5 driver. The Tx datapath shows an increase by 30.5\%, an inherent cost of deferring free page (Figure~\ref{fig:tx_exp_memory_tx}).
However, Figure~\ref{fig:tx_exp_memory_total} shows that when accounting for both Tx and Rx-datapath active pages, the total page count increases by only 3.28\% over vIOMMU-off,
...
As shown in Figure~\ref{fig:rx_exp_memory_total}
\name{} requires only 2.23\% more total pages over vIOMMU-off.
The active page count for Tx datapath only (Figure~\ref{fig:rx_exp_memory_tx}) shows a larger relative difference,
