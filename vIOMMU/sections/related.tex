...
\label{sec:related}
% We discuss key works related to {\name}.
% \brand is the first work that studies IO memory protection in virtualized environments with nested transaction, on modern server hardware architectures.
...
{\name} builds upon these works for the bare metal case. For example, persistent
mapping for receive buffers has been adopted by Linux (Rx datapath optimizations in \S\ref{sec:background-datapath}); similarly, as discussed in \S\ref{sec:impl}, we use the techniques developed in~\cite{Rubin2024FastSafe}. As shown in Figure~\ref{fig:ablation}, these mechanisms are helpful but not sufficient in virtualized environments---the key focus of our work. Indeed, we show in \S\ref{sec:inefficiencies} that the performance bottlenecks are quite different in virtualized environments and in fact may require design choices that make very different tradeoffs (\textit{e.g.}, {\name} reduces the number of VM exits by increasing the address translation cost).
...
% mapping for receive buffers has been adopted by Linux (Rx datapath optimizations in \S\ref{sec:background-datapath}).
...
%     to VM exits. \brand builds on the bare-metal foundations and reduces VM exits due to 
%     cache invalidations, the dominant bottleneck as link speeds increase.

% \brand requires no IOMMU hardware modification and is 
...
% Faster IOMMU hardware would further amplify gains of \brand.
...
% These approaches are complementary to \brand: reducing the hardware 
% cost of each invalidation could further amplify the gains from \brand's software-side batching and serialization elimination.
...
Amit et al.\ presented an vIOMMU emulation for
    ...
    memory pinning~\cite{Tian2020coIOMMU} and interrupts~\cite{Gordon2012eli, Har2013paraIO}. All the above work was conducted at 10--100\,Gbps. \brand extends these works in two ways. First, it presents new insights related to performance bottleneck at 400\,Gbps (\S\ref{sec:inefficiencies}); and second, {\name} addresses these bottlenecks with software techniques that require no paravirtualization or hardware modifications.
...
% \brand uses a push-based design where vCPUs enqueue invalidations into per-CPU buffers without waiting
% or dedicating a core. Optimistic teardown recovers line rate at 10\,GbE,
%     but weakens security guarantees; \brand preserves strict safety.
...
% is orthogonal to the IOTLB invalidation serialization that \brand addresses.  
...
% These optimizations are complementary to \brand, which focuses on the IOMMU emulation path rather than the interrupt delivery path.




\para{virtIO IOMMU.} \brand targets hardware-assisted vIOMMU, not virtio-IOMMU;
    despite that, the {\name} ideas may apply.
...
%    \brand eliminates vIOMMU overhead while maintaining strict safety protection.  
