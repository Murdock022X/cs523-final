...
The key driving principle in {\name} may be counter-intuitive: 
...
{\name} reduces the number of VM exits by explicitly increasing the cost of address translation. 

{\name} realizes the above principle using the key idea of \textit{one-for-many invalidations}. 
...
The one-for-many invalidations in {\name} design, as we discuss below, 
    ...
    However, the one-for-many invalidations in {\name} design will opportunistically execute one 
...
We implement \brand on Linux v6.12.9 with $\sim$1.5K lines of C code (350 in the device driver), 
...
Our evaluation results show that {\name} consistently achieves performance within 1--8$\%$ 
    ...
    {\name} thus expands the choices of real-world systems: high performance 
