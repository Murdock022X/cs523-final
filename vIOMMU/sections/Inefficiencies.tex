\section{Understanding Protection Overhead}
...
Every vCPU of the VM is pinned to a separate physical core on the host to avoid interference. See \S\ref{sec:eval} for more details.
...
\paragraphb{[Figure~\ref{fig:baseline-iperf}] Performance degradation with IO memory protection in VMs}
% Figure~\ref{fig:motivation} compares network throughput with increasing number of cores\saksham{should we use vCPUs, or cores in our discussion?} for three configurations: (1) vIOMMU disabled, (2) \sout{vIOMMU strict mode with shadow page tables,} and (3) vIOMMU enabled with strict memory protection and nested translation.
% Figure~\ref{fig:motivation-rx} and \ref{fig:motivation-tx} compare network throughput with increasing number of flows (each flow pinned to a core in the VM)
%   for two configurations: (1) vIOMMU disabled and (2) vIOMMU enabled with strict memory protection and nested translation.
Figure~\ref{fig:motivation-tx} shows throughput for the Tx-server.
...
Figure~\ref{fig:motivation-rx} shows throughput for the Rx-server.
...
Compared with the Tx-server case, the relatively higher Rx-server throughput is due to the page pool optimization that reduces map/unmap operations (\S\ref{sec:background-datapath}); and TCP ACK coalescing, which increases the number of pages 
...
\paragraphb{[Figure~\ref{fig:lock-contention}] Root cause of performance degradation with IO memory protection: 
...
\CodeIn{unmap} is far more costly than \CodeIn{map}: 
  ...
  require VM exits (\S\ref{sec:background-datapath}).
We observe that overheads of unmap operations
  ...
  %   We optimized the behavior in \S\ref{sec:impl}. \rachit{I suggest removing this footnote}}
...
%
Figure~\ref{fig:lock-contention} shows the average IR processing latency per \CodeIn{unmap}
  and its breakdown across different operations involved in IR-SUB datapath (\S\ref{sec:background-datapath}). 
We observe that, as the number of cores increases, IR processing latencies increase. This increase in IR processing latency is rooted in two key characteristics of the existing IR processing path (phase \BC{3} in \S\ref{sec:background-datapath}): 
...
Figure~\ref{fig:lock-contention} shows the consequence: 
...
Importantly, a majority time out of $L$ (>90\%) is spent on the {\it unavoidable VM exit} 
  required to submit the IR to the IOMMU invalidation queue in the host (see \S\ref{sec:sec:background-datapath}); 
...
% \label{fig:vm-exit-count}
% \end{figure}

% \todo{LESHNA: add pause as separate in vm exit count}

\paragraphb{[Figure~\ref{fig:lock-contention}] Root cause of throughput reduction at large core counts: % in Figure~\ref{fig:baseline-iperf}: 
...
% VM exits can happen due to many reasons; Figure~\ref{fig:vm-exit-time-pct} shows the average time spent on VM~exits on a per \CodeIn{unmap} basis, broken down by events that trigger the VM exit.
% due to all VM exits. 
...
and the total per-\CodeIn{unmap} exit cost is ${\sim}$11--18\,$\mu$s (IR-SUB cost in Figure~\ref{fig:lock-contention}).
...
% \rachit{It would also be nice to show the number of VM exits in Figure~\ref{fig:vm-exit-time-pct} to further support the evidence.}
