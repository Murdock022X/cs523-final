...
% \brand to achieve close-to-no-vIOMMU performance.
...
We implement \brand within Linux v6.12.9 in 1.9K lines of C code. 
...
\brand requires no modification to the hypervisor 
...
% \tianyin{Will need to rewrite after \S\ref{sec:design}}
...
% We implemented {\optOneHeading} (\S\ref{ssec:one-to-many}) with the following optimizations.
...
A producer must not return 
    ...
    \brand uses a sequence-based mechanism to ensure this invariant,
...
% In \S\ref{sec:design}, we have a high-level discussion 
...
% \subsection{Batched Multi-Range Unmap}
...
% \brand introduces a \emph{sub-gather} mechanism.  Rather than
...
% As described in \S\ref{sec:design}, DFP removes the producer's busy-wait after IR-PREP
...
% \brand dispatches each CPU's callbacks back to the originating CPU,
...
% \brand restructures this. Before initiating unmaps, the driver
...
\begin{figure*}[t]
    ...
    \end{subfigure}
    \vspace{-10pt}
    \caption{\small {\bf \brand enables high-performance networked applications to
    ...
    % Enabling vIOMMU causes up to 99\% throughput degradation. \brand delivers close-to-vIOMMU-off performance, with at most an 8\% throughput gap.
    \label{fig:apps}
    \vspace{-5pt}
\end{figure*}
...
% Because the same lock is contended during unmap-side invalidation (\S\ref{sec:inefficiencies}),
...
\brand precomputes these attributes and skips the lock acquisition at map time.
