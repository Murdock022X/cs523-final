\section{Background: DMA Datapath in Virtualized Environments}
\label{sec:background-datapath}
This section gives an overview of the DMA datapath 
    in virtualized environments with strict IO memory protection.
% Figure ~\ref{fig:datapath}  illustrates the NIC-to-memory datapath 
...
Figure~\ref{fig:datapath} illustrates the lifetime of a DMA operation under IO memory protection, 
...
The IOMMU provides an invalidation queue as an interface for the OS 
    to submit {\it invalidation requests (IRs)}. This happens in two steps (see Figure~\ref{fig:ir-ops}). 
...
% However, it becomes significantly more expensive in virtualized environment (see \S\ref{sec:pointer}).
...
% \subsubsection{Tx Datapath}
% \label{sec:tx-datapath}
...
% % \item 
% \subsubsection{\bf Rx Datapath.}
% \label{sec:rx-datapath}
...
% \leshnaedit{In our setup \S\ref{sec:inefficiencies}, over 99.9\% IOMMU operations on the Rx data path are skipped by page pool.}
