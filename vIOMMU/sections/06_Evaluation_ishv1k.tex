...
This section presents {\name} evaluation. We first discuss {\name} benefits for three real-world applications (\S\ref{ssec:real-world}), and then dive deeper into understanding {\name} benefits using microbenchmarks (\S\ref{ssec:micro}). Next, we perform an ablation study of {\name} design ideas (\S\ref{ssec:ablation}), discuss {\name} scalability with increasing number of VMs (\S\ref{sec:eval-multi-vm}), and finally also discuss any associated costs of using {\name} (\S\ref{ssec:cost}). 
% compared with vanilla system with vIOMMU on and off. 
% We use the pinned consumer configuration of {\name}.
Unless stated otherwise, the system setup is the same as described in \S\ref{sec:inefficiencies}. 
% \saksham{Comback here to add overview}

\para{Measurement setups.}
% \schaic{All comments resolved. @Rachit please check.}
As mentioned in \S\ref{sec:inefficiencies}, we use two servers with Emerald Rapids processor architecture (which supports nested translation) directly connected to each other via $400$Gbps Nvidia ConnectX-7 NICs; this is to ensure that performance bottlenecks are not in the network. 
% The servers have  
...
%   (full-system results in \S\ref{sec:eval}). 
...
% \schai{: receive (Figure~\ref{fig:motivation-rx}) and transmit (Figure~\ref{fig:motivation-tx}).
% \noindent
...
% \ikdelete{ improvements provided by \name}
% With \name, we closely match the vIOMMU Off performance.
% We then analyze microbenchmark results for both Rx and Tx traffic to better understand the sources of degradation and how \name mitigates them.
% Finally, we examine how each component of \name\ contributes to overall performance, 
% as well as its resource overhead and the effect of key design choices.

\subsection{{\name} Benefits for Real-world Applications}
\label{ssec:real-world}
We show the benefits of using {\name} for three widely used high-performance 
    networked applications: 
    NGINX (a web server), 
    Redis (a distributed key-value store), 
    and SPDK (a userspace storage stack that uses Linux TCP stack for remote storage).
We use the same setup as earlier along with $9$KB MTU size. This helps maximize the application performance and avoid CPU becoming the performance bottleneck, when running with vIOMMU off. This allows us to focus mainly on IO memory protection-induced overheads when enabling vIOMMU and/or {\name}. 
...
% enabling a robust evaluation of {\name} across traffic patterns.
...
Figure~\ref{fig:nginx} shows over {$99\%$} throughput degradation with vIOMMU-on, 
    consistently across all evaluated webpage sizes.
\brand improves throughput by up to 147$\times$ over vIOMMU-on,
...
% Figure~\ref{fig:nginx} exhibits subpar
% \saksham{again, let's not use subjective terms---a value may be par for some and subpar for some :-). Just provide the results and let people infer.} 
% nginx Tx\saksham{no need to repeat "Tx" again} performance with vIOMMU on. 
% The throughput degradation is consistent across all webpage sizes, with over $99\%$ throughput degradation
% \saksham{same feedback as earlier for times vs percentage}\ikdelete{. This degradation is consistent across all webpage sizes}\saksham{repeated statement}. 
% Using {\name}, the throughput is restored to near-vIOMMU-off performance\saksham{instead of phrases like throughput is restored, use simpler to-the-point phrases, like \name provides close-to-vIOMMU-disabled performance}\ikdelete{, with strict invalidation policy enabled}\saksham{no need to repeat this again}.
...
Figure~\ref{fig:redis} shows that vIOMMU-on
...
\brand improves throughput by up to 74$\times$ over vIOMMU-on 
...
\begin{figure*}[t]
    ...
    %     \label{fig:eval-core-unmap-ratio}
    % \end{subfigure}
    % \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
    ...
    \end{subfigure}
    \vspace{-10pt}
    \caption{\small {\bf Rx-server.} \name{} achieves up to $4.9\times$ Rx throughput improvement over vIOMMU-on.}
...
\begin{figure*}[t]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        ...
        \label{fig:tx-flush}
    \end{subfigure} 
%    \caption{Tx microbenchmark (one flow per core): \name{} achieves up to $36\times$ throughput improvement over the vIOMMU On configuration
%    by reducing IR-SUB count per page and IOTLB invalidation overhead.
    \vspace{-10pt}
    \caption{\small {\bf Tx-server.} \name{} achieves up to 36$\times$ Tx throughput improvement over vIOMMU-on.}
    \label{fig:tx}
    \vspace{-5pt}
\end{figure*}
...
Figure~\ref{fig:spdk} shows over 97\% throughput degradation 
    with vIOMMU-on across all block sizes.
\brand improves throughput by up to 82$\times$ over vIOMMU-on and 
...
% \vspace{0.1in}
% \noindent
% The small, 6-8\%, performance gap is observed only for the smallest-sized IO requests in all three applications. This is because CPU became the bottleneck for these cases even for vIOMMU-off, and additional CPU cycles spent on map/unmap operations with {\name} led to slightly lower throughput. 
...
\label{ssec:micro}
% Section~\ref{sec:inefficiencies} presents the overhead of memory protection with vIOMMU.

To better understand the reasons for {\name} performance benefits, we also evaluate {\name} using microbenchmarks. We use Iperf3~\cite{iperf3} to generate network traffic and measure throughput. We use similar setup as in \S\ref{sec:inefficiencies}, running same sets of controlled experiments, with the VM acting as the Tx-server and Rx-server, respectively. This enables us to analyze the individual impact of these setups on {\name} performance.



\para{Rx-server.} 
% \ikdelete{ We evaluate this setup under the three configurations described in \S\ref{sec:eval}}
...
% In contrast, \name restores throughput to near-vIOMMU-off level.
% \saksham{"restores"}
...
% Overall, \brand achieves up to a $83\%$ throughput improvement over the vIOMMU on configuration.
As shown in Figure~\ref{fig:eval-core-tput}, {\name} improves throughput by up to $4.9\times$ in comparison to vIOMMU-on, 
matching closely to vIOMMU-off performance (with a maximum gap of <6\%).
The key contributor to this throughput improvement is the reduction of IR-SUB operations per page transferred over network. As shown in Figure~\ref{fig:eval-core-qi}, {\name} reduces the IR-SUB count per page by up to 82\%. Recall from \S\ref{sec:background-datapath}, each IR-SUB operation requires an unavoidable VM exit, hence, reduction in IR-SUB count translates to a similar reduction in VM exits per page. {\name} achieves this reduction using two design ideas. One-for-many invalidations allow using a single IR-SUB to invalidate concurrently prepared IRs across multiple cores---hence, we see lower IR-SUB counts per page with increasing numbers of cores in Figure~\ref{fig:eval-core-qi}. Furthermore, deferred free page allows each core to prepare multiple IRs that can be concurrently processed by a single IR-SUB operation. This is achieved by avoiding busy waiting by each core until successful invalidation for each prepared IR, and hence reducing the time spent on IR processing by the core after {\tt unmap}. Figure~\ref{fig:eval-core-flush} shows the average time spent by application core on IR processing: while vIOMMU-on incurs large IR processing time due to incurring all---\CodeIn{cache\_lock} contention, IR-PREP and IR-SUB overheads,  {\name} avoids lock contention due to its per-CPU locks, and avoids IR-SUB due to deferred free page, and observes up to 99.9\% reduction in IR processing time. 

\para{Tx-server.} 
Figure~\ref{fig:tx-tput} shows that {\name} achieves up to 36$\times$ throughput improvement over vIOMMU-on,
closely matching vIOMMU-off (with the largest gap of <9\%). The root cause of {\name} benefits are similar to those discussed earlier for the Rx-server case. We briefly discuss the reason behind the relatively larger throughput gains in the Tx-server case. This is because of larger number of unmaps per page performed in Tx datapath (recall that Linux's page pool optimization does not apply to Tx datapath; \S\ref{sec:background-datapath}). Due to much larger number of unmaps per page, {\name} is able to enqueue much larger number of IR descriptors into the IO-domain queue, which can be submitted using a single IR-SUB---hence further reducing the IR-SUB count per page (as evident from Figure~\ref{fig:tx-qi}). This leads to relatively larger reductions in VM exits per page for Tx-server case, improving the relative performance gains.
...
% reducing the IR-SUB count per page by $300\times$ (Figure~\ref{fig:tx-qi}).
% DFP further increases the batching factor by releasing producer threads from waiting for invalidation to complete,
% reducing the average time the guest spends in the IOMMU subsystem by over 200$\times$ (Figure~\ref{fig:tx-flush}).
...
%     higher IR-SUB count per page compared to the Rx case (Figure~\ref{fig:tx-qi}).
...
% the near-total reduction confirms that {\name} effectively eliminates both bottlenecks.
...
% As shown in Figure~\ref{fig:eval-core-flush}, {\name} reduces the average time the guest OS spends in the IOMMU subsystem for IOTLB invalidation by up to 99.9\%,
% almost completely eliminating invalidation overhead.
% This metric subsumes locking contention and IR-SUB operation in Figure~\ref{fig:lock-contention};
% the near-total reduction confirms that {\name} effectively eliminates both bottlenecks.
...
% \ikdelete{ We evaluate this setup under the three configurations described in \S\ref{sec:eval}}\saksham{last line is not necessary; already mentioned in setup}\ikdelete{. Enabling vIOMMU in strict mode significantly degrades performance compared to vIOMMU off}\saksham{this was already discussed earlier in S3, when discussing fig 2} In contrast, \brand restores\saksham{"restores"} throughput to near-vIOMMU-off level\ikdelete{ while preserving the same security guarantees as strict invalidation policy}\saksham{this repetitive; and even then also inconsistent, sometimes it says strict protection, sometimes strict invalidation policy}. Overall, \brand achieves up to a $83\%$ throughput improvement over the vIOMMU on configuration.

% A key factor behind the substantial throughput improvement is the reduction in VM exits, due to fewer invalidation requests. As shown in Figure~\ref{fig:eval-core-qi}, \brand lowers invalidations per page by up to $82\%$ through delegated invalidation: requests from multiple cores are aggregated, and a single core issues a batched invalidation request. Importantly, a batched invalidation to the IOMMU hardware takes roughly the same time as an individual request, so the duration of each VM exit remains unchanged. Another contributing factor is the reduction in the total time the guest OS spends in the IOMMU subsystem, enabled by the deferred free page (DFP) optimization. During a VM exit, only the issuing core is blocked while waiting for the invalidation to complete, whereas other cores can simply enqueue their invalidation requests and continue execution without stalling. As shown in Figure~\ref{fig:eval-core-flush}, \brand reduces the average time that guests OS spends in the IOMMU subsystem by up to $99.9\%$, nearly eliminating the overhead of IOTLB invalidation.

% In Figure~\ref{fig:stress}, we stress-test \brand by varying the number of flows per core on a 24-core configuration. Combining results from Figure~\ref{fig:eval-core} and Figure~\ref{fig:stress}, we observe that \brand scales well with both increasing core counts and higher number of per-core flows. Across all scenarios, \brand closely matches vIOMMU off performance.
...
% A key factor behind the substantial throughput improvement is the reduction in VM exits, due to fewer invalidation requests. 
% As shown in Figure~\ref{fig:eval-core-qi}, \brand lowers invalidations per page by up to $82\%$ through delegated invalidation:
%  requests from multiple cores are aggregated, and a single core issues a batched invalidation request. 
%  Importantly, a batched invalidation to the IOMMU hardware takes roughly the same time as an individual request, so the duration of each VM exit remains unchanged. Another contributing factor is the reduction in the total time the guest OS spends in the IOMMU subsystem, enabled by the deferred free page (DFP) optimization. During a VM exit, only the issuing core is blocked while waiting for the invalidation to complete, whereas other cores can simply enqueue their invalidation requests and continue execution without stalling. As shown in Figure~\ref{fig:eval-core-flush}, \brand reduces the average time that guests OS spends in the IOMMU subsystem by up to $99.9\%$, nearly eliminating the overhead of IOTLB invalidation.



% \schaic{I have a figure for this (eval\_core\_exp-unmap\_to\_qi\_submit\_ratio.pdf). Shall we replace it with current Figure~\ref{fig:eval-core-qi}?}
...
% As we discussed in \S\ref{sec:inefficiencies},
...
% As shown in Figure~\ref{fig:eval-core-unmap-ratio}, 
...
% \name lowers invalidations per page by up to $82\%$ through delegated invalidation: 
...
\if 0
We further stress-test \name{} by scaling up to eight concurrent flows across 24 cores (see Figure~\ref{fig:stress}
    in the appendix).
Under excessive loads, \name{} maintains throughput within 1\% of vIOMMU-off performance.
Combining the results from Figures~\ref{fig:eval-core} and~\ref{fig:stress}, 
we observe that \name{} scales robustly with both increasing core counts and flows per core. 
\fi

% reduces the number of invalidation requests by over $300\times$ (Figure~\ref{fig:tx-flush}) through batching. 

% Combined with DFP, it also reduces the time the guest OS spends in the IOMMU subsystem by over $200\times$ (Figure~\ref{fig:tx-qi}), as cores can simply enqueue invalidation requests and avoid waiting for IOTLB invalidation to complete.


% \textbf{\name\ performance with Tx traffic.} We use the same setup as in \S\ref{sec:inefficiencies}. 
% In this experiment, the VM acts as sender and the client as receiver, with traffic generated using Iperf.
% Figure~\ref{fig:tx} evaluates the setup under the same three configurations as described in \S\ref{sec:eval}. 


% This is because the vIOMMU on configuration incurs a much higher number of invalidations per page for Tx traffic (Figure~\ref{fig:tx-qi}) than for Rx traffic (Figure~\ref{fig:eval-core-qi}). The increase is driven by more frequent \CodeIn{map()}/\CodeIn{unmap()} operations, as discussed in \S\ref{sec:linux-opt}, leading to more VM exits and greater time per core spent waiting on the \CodeIn{cache\_lock}.

% As shown in Figure~\ref{fig:tx-tput}, the throughput degradation between vIOMMU Off and the vIOMMU On configuration is significantly more pronounced in the Tx case. Despite this, \name restores throughput to near-vIOMMU-Off level. In fact, \name achieves up to $98\%$ throughput improvement over the vIOMMU On configuration. Similar to the Rx case, this improvement originates from two key optimizations---delegated invalidation and DFP optimization. Delegated invalidation reduces the number of invalidation requests by over $300\times$ (Figure~\ref{fig:tx-flush}) through batching. Combined with DFP, it also reduces the time the guest OS spends in the IOMMU subsystem by over $200\times$ (Figure~\ref{fig:tx-qi}), as cores can simply enqueue invalidation requests and avoid waiting for IOTLB invalidation to complete.
...
and other {\name} code optimizations (\S\ref{sec:other-opt}) to overall throughput for both Rx- and Tx-server setups.
In our analysis, we vary the number of cores from 4 to 24, 
    each running one flow, and report the corresponding throughput.

\para{Tx-server.} Figure~\ref{fig:ablation-tx} shows that code optimizations provide a modest contribution in overall gains. One-for-many invalidations improve the throughput further by {1.1-2.9$\times$}, with increasing gains with larger core count. This is expected since more cores enable {\name} to invalidate more concurrent IRs via single IR-SUB. Deferred free page, however, provides most significant contribution to throughput gains---{2.9 to 5.6$\times$}. Without deferred free page, each core stays blocked after IR-PREP until successful invalidation completion for the IR. When no longer constrained to busy wait with deferred free page, each core can continue execution of the future unmaps. And since in Tx-server setup, as discussed earlier, there are a large number of unmaps generated per core, this unblocking of cores results in significant throughput gains. 




% DFP is the primary contributor to throughput gains for Tx traffic.
% As discussed in \S\ref{sec:linux-opt},
...
% % , thanks to the page pool optimization (\S\ref{sec:linux-opt}).
...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{\columnwidth}
        ...
        \vspace{-5pt}
        \caption{Rx-server}
        \label{fig:ablation-rx}
    \end{subfigure}
    \vspace{-30pt}
    \caption{\small {\bf Necessity of both design ideas---one-for-many invalidations and deferred free page---to achieve {\name} benefits.}}
    \vspace{-10pt}
    \label{fig:ablation}
\end{figure}
...
% where datapath optimizations (\S\ref{sec:inefficiencies})\tianyin{???} reduce the number of invalidation requests,
...
% \brand can batch more than one request per core and issue invalidation requests with larger batch sizes.
% Overall, \brand recovers the majority of the remaining throughput in the Tx case,
...
\subsection{{\name} Scalability with Increasing VM count}
\label{sec:eval-multi-vm}

Cloud deployments commonly deploy more than one VM per server.
We evaluate how {\name} performance fares with an increasing number of VMs.
...
Figure~\ref{fig:multi-vm} shows that {\name} maintains close-to-vIOMMU-off performance for each evaluated VM count,
...
\begin{figure}[t]
    ...
    \vspace{-0.1in}
    \caption{\small {\bf {\name} maintains its benefits with increasing number of VMs per host. Discussion in \S\ref{sec:eval-multi-vm}}}
%    \vspace{-0.1in}
    \label{fig:multi-vm}
    \vspace{-0.1in}
\end{figure}

% \saksham{Can we get rid of this-->}This is a direct consequence of the intra-VM contention structure identified in \S\ref{sec:inefficiencies}:
...
% {\name} completely eliminates IOMMU overhead and scales well across multi-VM configurations, delivering $5.2$--$24\times$ improvement over vIOMMU-on.

\subsection{No Free Lunch: The Cost of {\name} Benefits}
\label{ssec:cost}
{\name} benefits come with two costs: it requires a dedicated core, and it requires extra memory due to deferred free page. We believe the former is not fundamental: it should be possible to opportunistically distribute the IR-SUB processing to producer threads (\textit{e.g.}, any lightly-loaded producer can become a consumer, and execute IR-SUB on its local core). Nevertheless, even with one dedicated core, {\name} expands the choices of real-world systems: high performance but with weak (or no) IO memory protection, abysmal performance with \textit{significantly underutilized} cores with strict IO memory protection, or, high performance with one dedicated core with strict IO memory protection. For the extra memory, our evaluation results presented in the Appendix-B suggest that the extra memory requirements are less than $4\%$ across all our evaluated workloads.

% The cost of achieving {\name} benefits is an additional core. 


% Cloud deployments commonly partition physical resources among multiple VMs. We evaluate how \name{} performs as the same 32 cores are divided among an increasing number of VMs---from a single 32-vCPU VM to sixteen 2-vCPU VMs.
...
% Figure~\ref{fig:multi-vm} shows that \name{} matches vIOMMU-off
% performance at every VM count.
% In contrast, enabling vIOMMU without \name{} causes
...
% This is a direct consequence of the per-VM contention structure identified in \S\ref{sec:inefficiencies}:
% each VM has its own emulated invalidation queue,
...
% \name{} eliminates this bottleneck entirely,
...
% \subsection{Impact of Consumer Placement.}
% \todo{@Siyuan Add text}
